...
| `OldPrivateSshKey` | Secret | Previous private SSH key | Managed by SPP from the vault |
...
```json
{
  "Parameters": [
    ...
    {"Name": "OldSshKey", "Type": "String"},
    {"Name": "OldPrivateSshKey", "Type": "Secret"}
  ]
}
```
...
| `PrivilegedPassword` | Secret | Privileged user password | Privileged-mode password |
...
| `ClientSecret` | Secret | API key secret stored in SPP | Secret value being checked or pushed |
